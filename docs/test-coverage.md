...
- `ContextReusableLayerTest` / `ContextRegistryContractTest` / `ContextValidationTest` / `ContextSystemCheckTest` / `ContextNoCallbackTest` / `ContextQueryContractTest` / `ContextConvenienceCompatibilityTest` / `ContextAuthQueryParityTest` — issue #39 first slice: model-free `trusts.context` (no `Trust` / `Content` / `Junction` nouns); direct and multi-hop related resolution; registry completeness, duplicate, freeze, deterministic order; scalar / many-valued / wrong-terminal / cyclic / missing paths fail closed; `trusts.E006`; getters and properties are not executed; exact one-query exists/list equivalence; Content, dependent Content, Junction, and Trust-as-Content lookups match the registry; `has_perm` / `.permitted()` agree; `from trusts.models import Junction` still works. Does not close #39
