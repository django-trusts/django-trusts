...
- `DefaultConfiguredModelContractTest` — issue #26 default-model regression: Role / TrustUserPermission / TrustGroup FKs use auth models; mismatched permission instances are not coerced by PK; `trusts.E003`–`E007` do not fire
- `ConfiguredModelConventionShapeTest` — issue #26 system-check shape: scalar `Group.user`, `user` FK to another model, `permissions` reverse query name other than `group`, and `content_type` that is not a `ContentType` relation are `trusts.E005` / `E004` / `E006`; a same-pk User vs OtherPrincipal FK named `user` shares Django's related-filter pk/attname and still cannot grant through Trusts
- Isolated `python -m tests.runtests_custom` — issue #26 fresh install with `TRUSTS_ENTITY_MODEL` / `TRUSTS_GROUP_MODEL` / `TRUSTS_PERMISSION_MODEL` selected before migrate: every Trusts relation targets the configured tables; trustee grants; group-permission ceiling; role-derived ceiling without `auth.Group` / `auth.Permission`; `has_perm` / `.permitted()` parity; TrustGroup grant/set/revoke and authorization helpers; mismatched instances fail closed
- `scripts/verify-custom-role-upgrade.py` — issue #26 colliding-PK upgrade: Role joins against `auth.Group` / `auth.Permission` that share primary keys with custom models are cleared by `0003` before retargeting
