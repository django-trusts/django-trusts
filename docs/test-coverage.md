...
- `DefaultConfiguredModelContractTest` — issue #26 default-model regression: Role / TrustUserPermission / TrustGroup FKs use auth models; mismatched permission instances are not coerced by PK; `trusts.E003`–`E005` do not fire on the supported contract; lookup failures keep distinct IDs; silenced `trusts.E003` / `E004` / `E005` still fail closed at grant and `has_perm` / `.permitted()`; silenced `trusts.E003` also denies an existing Group membership + ceiling + local TrustGroup grant
- Isolated `python -m tests.runtests_custom` — issue #26 fresh install with custom `AUTH_USER_MODEL` / `TRUSTS_ENTITY_MODEL` selected before migrate: settlor/trustee FKs target the custom user table; Group and Permission remain `auth.*`; trustee grants; group-permission ceiling; role-derived ceiling; `has_perm` / `.permitted()` parity; TrustGroup grant/set/revoke and authorization helpers; mismatched instances fail closed
