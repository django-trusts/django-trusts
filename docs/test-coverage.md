...
- `ConfiguredModelConventionShapeTest` — issue #26 system-check shape: scalar `Group.user`, `user` FK to another model, `permissions` reverse query name other than `group`, and `content_type` that is not a `ContentType` relation are `trusts.E005` / `E004` / `E006`; a same-pk User vs OtherPrincipal FK named `user` shares Django's `.pk` prep value and still cannot grant through Trusts
