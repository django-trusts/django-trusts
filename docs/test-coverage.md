...
- `python -m tests.runtests` — discovers the core and regression modules
  ...
  `tests.core.test_wheel_install`, `tests.regressions.test_issue_*`)
...
- `ContextReusableLayerTest` / `ContextRegistryContractTest` / `ContextValidationTest` / `ContextSystemCheckTest` / `ContextNoCallbackTest` / `ContextQueryContractTest` / `ContextConvenienceCompatibilityTest` / `ContextAuthQueryParityTest` — issue #39 first slice: model-free `trusts.context` (no `Trust` / `Content` / `Junction` nouns); direct and multi-hop related resolution; registry completeness, duplicate, freeze, deterministic order; public `Content.register_content` / `Junction` are idempotent-only or rejected after freeze; `related_name='+'` and partial `UniqueConstraint(condition=...)` fail closed at registration and as `trusts.E006`; scalar / many-valued / wrong-terminal / cyclic / missing paths fail closed; installed stale adapters emit `trusts.E006` (id/object/hint); getters and properties are not executed; exact one-query exists/list equivalence; Content, dependent Content, Junction, and Trust-as-Content lookups match the registry; `has_perm` / `.permitted()` agree; `from trusts.models import Junction` still works. Does not close #39
