...
- `OnePathAuthorizationTest` / `DistributiveLawTest` / `CompilerIsolationBackendTest` / `CoordinatorQueryCountTest` / `ObjNoneBoundaryTest` / `UndeclaredJunctionRemainsHistoricalTest` / `CompilerFailureTest` — issue #77 S3b: declared Category/Ticket/Trust `has_perm` / `has_perms` / `get_all_permissions` / `get_group_permissions` on the path-scoped compiler (trustee and historical group-only; wrong perm / other Trust deny; conditions overlay; object and QuerySet parity); distributive-law A-grants-C1 / B-grants-C2 (Django/object grant each row, aggregate QuerySet `has_perm` true, common enumeration contains P, `.permitted` both in 1 SQL); failed ceiling cannot borrow another path's fragment; concrete group route grants and mixin-only plan route denies; coordinator vs non-coordinator query counts including reversed order and duplicate exact path strings; `get_group_permissions` is group-only; Trusts-only `obj is None` is false/empty and `ModelBackend` + Trusts restores globals without object grants; undeclared Junction/Group stay historical; malformed/raising/unconfigured/ambiguous compilers fail loud. No schema or migration.
