...
- `TrustsRegistryTest` — issue #57: isolated `TrustsRegistry` / root-relative `Ref` registration only (ordinary Document / DocumentGrant; inferred terminals; mixed-root / missing / scalar / multi-valued / reverse / GFK / non-model / duplicate / conflict; zero queries). Not an authorization test
