"""#16 / #142 Stage B: per-handle condition registry and builders.

Library-only proofs. Does not import Zero nouns. Public registration
accepts builders only; prebuilt ``Expr`` raises ``TypeError`` before
mutation. Compiler coverage uses ``trusts.conditions._ir``.
"""
...
def _store_expr(registry, model, cond_code, expr):
    """Plant normalized IR without the public registration API."""
    store = getattr(registry, 'conditions', registry)
    record = ConditionRecord(expr=expr, model=model)
    store._records[(model._meta.label, cond_code)] = record
    return record
...
class ConditionRegistryIsolationTest(SimpleTestCase):
    def test_no_process_global_condition_store(self):
        ...
        self.assertFalse(hasattr(conditions_mod, 'Expr'))
        self.assertFalse(hasattr(conditions_mod, 'condition_refs'))
        self.assertFalse(hasattr(conditions_mod, 'Query'))
        self.assertFalse(hasattr(conditions_mod, 'TQ'))
...
class ConditionRegistryShapeTest(SimpleTestCase):
    def test_expr_and_builder_records_and_shape_errors(self):
        ...
        self.assertIs(
            registry.get_permission_condition_record(Note, 'spy'), builder_record,
        )
        ...
        self.assertEqual(len(log.calls), 1)
        self.assertIsNone(registry.get_permission_condition_record(Note, 'bare'))
        self.assertIsNone(registry.get_permission_condition_record(Note, 'bad'))
        self.assertIs(
            registry.get_permission_condition_record(Note, 'spy'), builder_record,
        )
    ...
    def test_handle_register_forwards_and_freeze_is_before_builder(self):
        ...
        u, _p, o = condition_refs()
        with self.assertRaises(TypeError) as ctx:
            handle.register_permission_condition(Note, 'prebuilt', u == o.owner)
        self.assertIn('builder callable', str(ctx.exception))
        self.assertIsNone(registry.get_permission_condition_record(Note, 'prebuilt'))
        self.assertIs(
            registry.get_permission_condition_record(Note, 'own'), record,
        )
        registry.freeze()
...
class ConditionRegistryRuntimeTest(TransactionTestCase):
    ...
    def test_core_registration_path_is_zero_sql_for_builder_and_expr_reject(self):
        Note, _Memo = _note_models()
        u, _p, o = condition_refs()
        registry = TrustsRegistry()
        with self.assertNumQueries(0):
        ...
        self.assertIsNone(registry.get_permission_condition_record(Note, 'expr'))
        with self.assertNumQueries(0):
            registry.register_permission_condition(
...
class ConditionRegistryCheckTest(SimpleTestCase):
    ...
    def test_invalid_stored_ir_is_check_error(self):
        Note, _Memo = _note_models()
        u, _p, o = condition_refs()
        registry = ConditionRegistry()
        expr = u == o.not_a_field
        record = _store_expr(registry, Note, 'missing', expr)
