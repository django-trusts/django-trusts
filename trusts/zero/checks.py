"""Zero system checks for Content conditions and auth-model contract.

Kernel adapter re-walks (``trusts.E006`` frozen Context adapters,
``trusts.E007``) live in ``trusts.checks``. This module owns
``trusts.E001``–``E005`` and ``W001``–``W003``.
"""
...
from trusts.zero.models import Content, legacy_permission_callbacks_allowed
