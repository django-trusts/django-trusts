...
python scripts/verify-custom-role-upgrade.py
```

CI is GitHub Actions (`.github/workflows/ci.yml`): authorization tests, a
fresh migrate, ``manage.py check``, the isolated custom-model suite, the
legacy-upgrade script, and the colliding-PK custom Role upgrade script on
Python 3.12, 3.13, and 3.14 with Django 6.1. The `package` job (Python 3.12 only) builds an sdist/wheel
